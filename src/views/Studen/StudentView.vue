  ...
  <div class="flex h-screen bg-gray-50 dark:bg-gray-950">
    ...
    <main class="flex-1 overflow-y-auto">
      <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-10 py-6 lg:py-10">
        ...
        <!-- Encabezado principal (sólido, sin tarjetas) -->
        <header class="mb-10 border-b border-gray-200 dark:border-gray-800 pb-6">
          ...
          <p class="mt-2 text-base sm:text-lg text-gray-600 dark:text-gray-400">
            Aquí tienes una vista clara de tu avance académico.
          </p>
        </header>

        <!-- Sub-nav pegajosa (sólida) -->
        <nav
          class="sticky top-0 z-10 -mx-4 sm:-mx-6 lg:-mx-10 px-4 sm:px-6 lg:px-10 py-3 bg-gray-50/95 dark:bg-gray-950/95 backdrop-blur border-y border-gray-200 dark:border-gray-800"
          aria-label="Secciones del tablero"
        >
          <ul class="flex items-center gap-3 text-sm">
            <li>
              <a href="#cursos" class="chip">Cursos</a>
            </li>
            <li>
              <a href="#tareas" class="chip">Tareas</a>
            </li>
            <li>
              <a href="#rendimiento" class="chip">Rendimiento</a>
            </li>
          </ul>
        </nav>
        ...
        <div class="mt-8 space-y-14">
          ...
          <!-- Sección: Cursos (lista sólida, sin tarjetas) -->
          <section id="cursos" aria-labelledby="cursos-title">
            ...
            <!-- Lista tipo “line items” con líneas y estados hover -->
            <div class="mt-4">
              <ul class="divide-y divide-gray-200 dark:divide-gray-800">
                ...
                >
                  ...
                  >
                    Ver detalles
                    <svg xmlns="http://www.w3.org/2000/svg" class="ml-1 h-4 w-4 transition-transform group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          ...
          <!-- Sección: Tareas (timeline simple, sin tarjetas) -->
          <section id="tareas" aria-labelledby="tareas-title">
            ...
            <!-- Timeline vertical (línea sólida a la izquierda) -->
          ...
          <!-- Sección: Rendimiento (tabla compacta, sólida, sin tarjetas) -->
...
import Sidebar from './Sidebar.vue' // mantiene tu estructura y base de datos sin tocar

const userStore = useUserStore()

// Bienvenida dinámica desde el store
const studentName = computed(() => userStore.user?.full_name || 'Estudiante')

// Datos (placeholders de ejemplo; en tu app vendrán de la BD/API)
const courses = ref([
...
<style scoped>
/* chips del subnav (sólidas, sin tarjetas) */
.chip {
...
/* Encabezados de sección: icono sólido + título, sin card */
.section-head {
...
/* Filas de listas (sin tarjetas) */
.item-row {
  @apply flex items-center justify-between py-3 px-2 rounded hover:bg-gray-100 dark:hover:bg-gray-900 transition;
}

/* Pills de vencimiento (sólidas) */
.pill {
  @apply text-[10px] sm:text-xs font-bold px-2.5 py-1 rounded-full whitespace-nowrap;
}
...
/* Tabla sólida y compacta */
.table-th {
  @apply px-4 py-2 text-left text-xs font-semibold uppercase tracking-wide;
}
.table-td {
